    ...
    return new Promise((resolve, reject) => {
        recursiveScan(rootUrl, [".jpg", ".png", ".jpeg", ".gif", ".png"])
            ...
            .on("data", (data) => {
                ...
                sp.setMessage(
                    `Recursively scanning ${rootUrl}, ${stats.filtered} / ${
                        stats.filtered + stats.removed
                    }`
                );
            })
            .on("finish", () => {
            ...
            exif.parse(filePath, (err, data) => {
                ...
                callback(null, {
                    ...
                    meta: {
                        exif: data,
                        tags: path
                            .dirname(filePath.replace(rootUrl, ""))
                            .split("/")
                            .filter((x) => x.length > 0)
                    }
                });
...
const omitIndexed = (alreadyIndexed) =>
    new Transform({
        transform(chunk, encoding, callback) {
            const filePath = String(chunk);
...
async function loadIndexedFilenames() {
    ...
    try {
        const result = await client.query(
            "select path from photo_scanner.photos"
        );
...
async function main(pool, rootUrl) {
    ...
    const processedPhotos = pipeline(
        readableStream,
        omitIndexed(await loadIndexedFilenames()),
        exifExtractor(rootUrl),
        (err) => {}
    );
    var x = 0;
    for await (const processed of processedPhotos) {
        sp.setMessage(
            `Processed ${processed.filePath} (${++x}/${filesObject.filtered})`
        );
        const client = await pool.connect();
        try {
            await client.query(
                ...
                [
                    uuid.v4(),
                    path.basename(processed.filePath),
                    processed.filePath,
                    processed.meta
                ]
            );
